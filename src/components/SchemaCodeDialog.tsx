...
interface SchemaCodeDialogProps {
  open: boolean
  onClose: () => void
...
function highlightLine(
  ...
  // We'll do a simple regex-based approach
  ...
  while ((match = tokenRegex.exec(mainPart)) !== null) {
    ...
    if (match[1]) {
      // Keyword (export, const, import, from)
      segments.push({text: match[0], className: 'schema-code-keyword'})
    } else if (match[2]) {
      // Function name (defineType, defineField, defineArrayMember)
      segments.push({text: match[0], className: 'schema-code-function'})
    } else if (match[3]) {
      // String literal — check if it contains a reference to a known type
      const strContent = match[3].slice(1, -1) // remove quotes
      ...
      // Property name (name, title, type, fields, to, of)
